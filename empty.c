...
#include <stdlib.h>
//测试用的回调函数
void Button_1_Mode(pClass_Bluetooth this)
{
	this->Send_Datas(this,(uint8_t *)"Mode_1\n\r",8);
}

void Button_2_Mode(pClass_Bluetooth this)
{
	this->Send_Datas(this,(uint8_t *)"Mode_2\n\r",8);
}

void Button_3_Mode(pClass_Bluetooth this)
{
	this->Send_Datas(this,(uint8_t *)"Mode_3\n\r",8);
}

void Button_4_Mode(pClass_Bluetooth this)
{
	this->Send_Datas(this,(uint8_t *)"Mode_4\n\r",8);
}

void Button_5_Mode(pClass_Bluetooth this)
{
	this->Send_Datas(this,(uint8_t *)"Mode_5\n\r",8);
}
...
//  
  ...
  Bluetooth_Debuger->Configure_Mode(Bluetooth_Debuger, STRING);                                   // 配置调试模式
...
//  Bluetooth_Debuger->Bind_Param_With_Id(Bluetooth_Debuger, 2, &(Motor_LF->Output_Now));         // 绑定参数2
  ...
  //按键配置
  pClass_AdcKey Adc_Button = Create_AdcKey();//创建按键对象
  Adc_Button->Init(Adc_Button,Bluetooth_Debuger);//初始化按键对象
  Adc_Button->Set_Callback(Adc_Button,Mode_1,Button_1_Mode);	//配置按键1的回调函数
  Adc_Button->Set_Callback(Adc_Button,Mode_2,Button_2_Mode);	//配置按键2的回调函数
  Adc_Button->Set_Callback(Adc_Button,Mode_3,Button_3_Mode);	//配置按键3的回调函数
  Adc_Button->Set_Callback(Adc_Button,Mode_4,Button_4_Mode);	//配置按键4的回调函数
  Adc_Button->Set_Callback(Adc_Button,Mode_5,Button_5_Mode);	//配置按键5的回调函数
  while (1)
  {
...
//    delay_ms(10);
	  ...
	  Adc_Button->Check_And_Trigger(Adc_Button);//检查状态并触发回调
