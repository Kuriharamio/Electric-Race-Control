...
{
  board_init();
  DL_GPIO_setPins(MOTOR_DRV_STBY_F_PORT, MOTOR_DRV_STBY_F_PIN); // 设置电机待机引脚
  // 电机配置
  ...
  Bluetooth_Debuger->Bind_Param_With_Id(Bluetooth_Debuger, 0, &(Motor_LF->Now_Speed));             // 绑定参数
  Bluetooth_Debuger->Bind_Param_With_Id(Bluetooth_Debuger, 1, &(Motor_LF->Target_Speed));             // 绑定参数
  Bluetooth_Debuger->Bind_Param_With_Id(Bluetooth_Debuger, 2, &(Motor_LF->Output_Now));             // 绑定参数
  Bluetooth_Debuger->Bind_Param_With_Id(Bluetooth_Debuger, 3, &(Motor_LF->Total_Encoder_Tick)); // 绑定参数
