...
pClass_Car Create_Car(void)
{
    ...
    car->PID_Follow = create_PID();            // 跟随PID
...
void Car_Init(pClass_Car this)
{
    ...
    this->PID_Follow->PID_Init(this->PID_Follow, 0.05, 15.0, 15.0, 0.05, 1.3, 1.3, PID_CAR_POSITION_TIMER_T, 0.02, 0.0, 0.0, 0.0, PID_D_First_ENABLE);                                          // 初始化PID参数
